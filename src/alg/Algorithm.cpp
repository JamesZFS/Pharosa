...
Color Algorithm::LdFaster(const Intersection &isect) const
{
	...
	auto shape = dynamic_cast<Finite *>(light->geo);
	assert(shape);	// todo remove this
	Ray r_out;
	r_out.org = isect.pos;
	real pdf = 1.f / N;
	real G = 1;
	...
	switch (light->geo->type()) {
		case Geometry::SPHERE: {
			...
			pdf /= 2 * M_PIF * (1 - cos_theta_max);
			break;
		}
		case Geometry::TRIANGLE: {
			auto &t = *(Triangle *) light->geo;
			Pos d = Sampling::uniformOnTriangle(t.p[0], t.p[1], t.p[2], {randf(), randf()}) - isect.pos;
			real dist = d.norm();
//			if (r_out.dir % t.n <= 0)
//				return Color::BLACK;
			r_out.dir = d / dist;
			G = (isect.nl % r_out.dir) * fabsf(r_out.dir % t.n) / (dist * dist);
			pdf /= 0.5 * t.area();	// only sample one surface
			break;
		}
		default:
			TERMINATE("Got unsupported finite shape light source!")
	}
	if (isect.nl % r_out.dir <= 0)
		return Color::BLACK;    // shadow ray
	r_out.offset(EPS);
...
//	return light->mtr->emi * (isect.nl % r_out.dir * 2 * (1 - cos_theta_max) * N);
	return light->mtr->emi * (M_1_PIF * G / pdf);
}
